...
class Script {
      ...
      switch (op.value) {
        ...
        case opcodes.OP_CHECKSIGVERIFY: {
          ...
          if (sig.length > 0) {
            ...
            res = checksig(hash, sig, key.slice(1));
          }
        ...
        case opcodes.OP_CHECKMULTISIGVERIFY: {
          ...
          while (res && m > 0) {
            ...
            if (sig.length > 0) {
              ...
              if (checksig(hash, sig, key.slice(1))) {
                isig += 1;
                m -= 1;
              }
            }
  ...
  static verify(input, witness, output, tx, index, value, flags) {
    ...
    //throw new ScriptError("flags, tx, index, value "+ flags+tx+index+value);
...
function checksig(msg, sig, key) {
  ...
  return falcon.verify(key, msg, sig.slice(0, -1));
}
