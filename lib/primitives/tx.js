...
class TX {
  ...
  checksig(index, prev, value, sig, key, version) {
    ...
    return falcon.verify(key,hash,sig.slice(0,-1));
  }
  ...
  signature(index, prev, value, key, type, version) {
    ...
    const sig = falcon.sign(key,hash);
  ...
  hasStandardWitness(view) {
    ...
    for (const input of this.inputs) {
      ...
      if (prev.isWitnessPubkeyhash()) {
        ...
        if (witness.items[0].length > policy.SIGNATURE_SIZE)
          return false;

        //if (witness.items[1].length > policy.PUBKEY_SIZE)
        if (witness.items[1].length > policy.PUBKEY_SIZE+1)
          return false;
      ...
      if (prev.isWitnessScripthash()) {
        ...
        if (m !== -1) {
          ...
          for (let i = 1; i < witness.items.length - 1; i++) {
            const item = witness.items[i];
            if (item.length > policy.SIGNATURE_SIZE)
              return false;
          }
        }
