...
const ZERO_KEY = Buffer.alloc(policy.PUBKEY_SIZE, 0x00);
...
class KeyRing {
  ...
  generate(compress) {
    const r_keys =  falcon.generateKeypairRandom();
    //const key = secp256k1.privateKeyGenerate();
    return this.fromKey(r_keys.priv, compress);
  }
  ...
  fromKey(key, compress) {
    ...
    if (key.length === policy.PRIVKEY_SIZE)
  ...
  getSecretSize() {
    ...
    if (this.publicKey.length === policy.PUBKEY_SIZE)
      size += 1;
  ...
  sign(msg) {
    ...
    return falcon.sign(this.privateKey,msg);
  ...
  verify(msg, sig) {
    return falcon.verify(this.publicKey.slice(1),msg,sig);
  ...
  toWriter(bw) {
    ...
    if (this.privateKey) {
      ...
      bw.writeU8(this.publicKey.length === policy.PUBKEY_SIZE ? 1 : 0);
    } else {
  ...
  fromReader(br) {
    ...
    if (key.length === policy.PUBKEY_SIZE) {
      const compress = br.readU8() === 1;
      this.privateKey = key;
      this.publicKey = falcon.privToPub(key);
