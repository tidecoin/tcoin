...
const falcon = require('tdc-falcon');
...
class HDPrivateKey {
  ...
  constructor(options) {
    ...
    this.childIndex = 0;
    this.chainCode = consensus.ZERO_FALCON_HASH;
    this.privateKey = common.ZERO_PRIVKEY;
  ...
  fromOptions(options) {
    ...
    this.publicKey = falcon.privToPub(options.privateKey); //secp256k1.publicKeyCreate(options.privateKey, true);
  ...
  derive(index, hardened) {
    ...
    //const bw = bio.pool(policy.PRIVKEY_SIZE+5);
    const bw = bio.pool(policy.PUBKEY_SIZE+4);
    ...
    const hash = pbkdf2.derive(sha512, data, this.chainCode, 50000, 96);
    const left = hash.slice(0, 48);
    const right = hash.slice(48, 96);
    ...
    try {
      ...
      keys=falcon.generateKeypair(left);
    } catch (e) {
    ...
    child.privateKey = keys.priv;
    child.publicKey = Buffer.concat([Buffer.from([0x07]),keys.pub]);
  ...
  fromSeed(seed) {
    ...
    const hash = pbkdf2.derive(sha512, seed, SEED_SALT, 50000, 96);
    const left = hash.slice(0, 48);
    const right = hash.slice(48, 96);
    ...
    // let xseed = Buffer.alloc(48,0x00);
    // left.copy(xseed,0,0,32);
    // keys=falcon.generateKeypair(xseed);
    keys=falcon.generateKeypair(left);

    this.privateKey = keys.priv;
    this.publicKey = Buffer.concat([Buffer.from([0x07]),keys.pub]);
  ...
  fromKey(key, entropy) {
    assert(Buffer.isBuffer(key) && key.length === policy.PRIVKEY_SIZE);
    assert(Buffer.isBuffer(entropy) && entropy.length === 48);
    this.depth = 0;
    this.parentFingerPrint = 0;
    this.childIndex = 0;
    this.chainCode = entropy;
    this.privateKey = key;
    this.publicKey = Buffer.concat([Buffer.from([0x07]),falcon.privToPub(key)]);
    return this;
  }
  ...
  static generate() {
    const keys = falcon.generateKeypairRandom(); //secp256k1.privateKeyGenerate();
    const entropy = random.randomBytes(48);
    return HDPrivateKey.fromKey(keys.priv, entropy);
  }
  ...
  fromReader(br, network) {
    ...
    assert(br.readU8() === 0);
    this.privateKey = br.readBytes(policy.PRIVKEY_SIZE);
    this.publicKey = Buffer.concat([Buffer.from([0x07]),falcon.privToPub(this.privateKey)]); //secp256k1.publicKeyCreate(this.privateKey, true);
  ...
  toRaw(network) {
    return this.toWriter(bio.write(66+policy.PRIVKEY_SIZE), network).render();
  }
