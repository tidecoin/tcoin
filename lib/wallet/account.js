...
class Account {
  ...
  getSize() {
    let size = 0;
    //size += 59+policy.PUBKEY_SIZE;//957;//policy.PUBKEY_SIZE+policy.PRIVKEY_SIZE+256;
    size += 75+policy.PUBKEY_SIZE;//957;//policy.PUBKEY_SIZE+policy.PRIVKEY_SIZE+256;
    size += this.keys.length * (policy.PUBKEY_SIZE+policy.PRIVKEY_SIZE);
    return size;
  }
...
function readKey(br) {
  ...
  key.chainCode = br.readBytes(48);
  key.publicKey = br.readBytes(policy.PUBKEY_SIZE);
  return key;
}
