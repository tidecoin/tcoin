...
class MasterKey {
  ...
  readKey(data) {
    ...
    if (isLegacy(data)) {
      br.seek(13);
      this.key.chainCode = br.readBytes(48);
      assert(br.readU8() === 0);
      this.key.privateKey = br.readBytes(policy.PRIVKEY_SIZE);
    } else {
      this.key.chainCode = br.readBytes(48);
      this.key.privateKey = br.readBytes(policy.PRIVKEY_SIZE);
    }

    this.key.publicKey = Buffer.concat([Buffer.from([0x07]),falcon.privToPub(this.key.privateKey)]);
  ...
  fromReader(br) {
    ...
    this.key.chainCode = br.readBytes(48);
    this.key.privateKey = br.readBytes(policy.PRIVKEY_SIZE);
    this.key.publicKey = Buffer.concat([Buffer.from([0x07]),falcon.privToPub(this.key.privateKey)]);//secp256k1.publicKeyCreate(this.key.privateKey, true);
